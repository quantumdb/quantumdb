...
public class Plan {
	...
	static class Builder {
		...
		public Plan build(RefLog refLog, Set<Table> ghostTables, Set<Function> newFunctions,
				Set<Function> removedFunctions) {

			return new Plan(Lists.newArrayList(steps), refLog, ghostTables, newFunctions, removedFunctions);
		}
	...
	private final ImmutableSet<Function> newFunctions;
	private final ImmutableSet<Function> removedFunctions;

	public Plan(List<Step> steps, RefLog refLog, Set<Table> ghostTables, Set<Function> newFunctions,
			Set<Function> removedFunctions) {

		this.steps = ImmutableList.copyOf(steps);
		this.refLog = refLog;
		this.ghostTables = ImmutableSet.copyOf(ghostTables);
		this.newFunctions = ImmutableSet.copyOf(newFunctions);
		this.removedFunctions = ImmutableSet.copyOf(removedFunctions);
	}
